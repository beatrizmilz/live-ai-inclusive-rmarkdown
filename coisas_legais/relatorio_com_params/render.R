...
rmarkdown::render(
  ...
  input =  here::here(
    "coisas_legais",
    "relatorio_com_params",
    "rmd_params_exemplo.Rmd"
  ),
  # o segundo argumento é a lista de parâmetros
  params = list(mes = 10, ano = 2021, sistema = "Rio Grande"),
...
sistemas <- unique(mananciais::mananciais$sistema)


fs::dir_create("coisas_legais/relatorio_com_params/output")
...
purrr::map(
  ...
  .f = ~ rmarkdown::render(
    ...
    output_file = paste0("output/", .x, ".html")
  )
)
